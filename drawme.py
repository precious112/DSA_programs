#a program that lets you draw different shapes and can be used to make sketches
import turtle
import tkinter
...
class RectangleCommand:
    def __init__(self,height,breadth,angle,width=1,color="black"):
        self.height= height
    ...
    def __str__(self):
        return '<Command height="' + str(self.height) + "' breadth='" + str(self.breadth) + "' angle='" + \
               '" width="' + str(self.width) \
               + '" color="' + self.color + '">GoTo</Command>'

class rainbowCommand:
	def __init__(self,radius1=80,val=10,width=1):
	...
	def draw(self,turtle):
		...
		turtle.circle((10+self.radius1),-180)
		turtle.penup()
		turtle.setpos((-10+self.val),0)
		turtle.pendown()
		turtle.right(180)
		turtle.pencolor('indigo')
		turtle.circle((20+self.radius1),-180)
		turtle.penup()
		turtle.setpos((-20+self.val),0)
		turtle.pendown()
		turtle.right(180)
		turtle.pencolor('blue')
		turtle.circle((30+self.radius1),-180)
		turtle.penup()
		turtle.setpos((-30+self.val),0)
		turtle.pendown()
		turtle.right(180)
		turtle.pencolor('green')
		turtle.circle((40+self.radius1),-180)
		turtle.penup()
		turtle.setpos((-40+self.val),0)
		turtle.pendown()
		turtle.right(180)
		turtle.pencolor('yellow')
		turtle.circle((50+self.radius1),-180)
		turtle.penup()
		turtle.setpos((-50+self.val),0)
		turtle.pendown()
		turtle.right(180)
		turtle.pencolor('orange')
		turtle.circle((60+self.radius1),-180)
		turtle.penup()
		turtle.setpos((-60+self.val),0)
		turtle.pendown()
		turtle.right(180)
		turtle.pencolor('red')
		turtle.circle((70+self.radius1),-180)
		turtle.penup()
		turtle.setpos((-70+self.val),0)
		turtle.pendown()
		turtle.right(180)
	def __str__(self):
		return '<Command radius1="' + str(self.radius) + '" width="' + \
               str(self.width) + '" val="' + self.val + '">Rainbow</Command>'
...
class DrawingApplication(tkinter.Frame):
    ...
    def buildWindow(self):
        ...
        def parse(filename):
            ...
            for commandElement in graphicsCommands:
                ...
                if command == "GoTo":
                    ...
                    radius1 = float(attr["radius1"].value)
                    width = float(attr["width"].value)
                    val = attr["val"].value.strip()
                    cmd = CircleCommand(radius1,width,val)

                    
                elif command == "Rectangle":
                    height = float(attr["height"].value)
                    breadth= float(attr["breadth"].value)
                    angle= float(attr["angle"].value)
                    width = float(attr["width"].value)
                    color = attr["color"].value.strip()
                    cmd = CircleCommand(radius,width,color)
        ...
        pointLabel = tkinter.Label(sideBar,text="radius1")
        pointLabel.pack()
        radius1Size = tkinter.StringVar()
        radius1Entry = tkinter.Entry(sideBar,textvariable=radius1Size)
        radius1Entry.pack()
        radius1Size.set(str(40))
        
        pointLabel = tkinter.Label(sideBar,text="val")
        pointLabel.pack()
        ValSize = tkinter.StringVar()
        ValEntry = tkinter.Entry(sideBar,textvariable=ValSize)
        ValEntry.pack()
        ValSize.set(str(40))
        def rainbowHandler():
            ...
            cmd = rainbowCommand(float(radius1Size.get()), float(widthSize.get()), float(ValSize.get()))
            cmd.draw(theTurtle)
            self.graphicsCommands.append(cmd)
        ...
        rainbowButton = tkinter.Button(sideBar, text = "Draw Rainbow", command=rainbowHandler)
        rainbowButton.pack(fill=tkinter.BOTH) 
        pointLabel = tkinter.Label(sideBar,text="height")
        pointLabel.pack()
        heightSize = tkinter.StringVar()
        heightEntry = tkinter.Entry(sideBar,textvariable=heightSize)
        heightEntry.pack()
        heightSize.set(str(50))
        
        pointLabel = tkinter.Label(sideBar,text="breadth")
        pointLabel.pack()
        breadthSize = tkinter.StringVar()
        breadthEntry = tkinter.Entry(sideBar,textvariable=breadthSize)
        breadthEntry.pack()
        breadthSize.set(str(40))
        ...
        def rectangleHandler():
            # When drawing, a command is created and then the command is drawn by calling
            # the draw method. Adding the command to the graphicsCommands sequence means the
            # application will remember the picture. 
            cmd = RectangleCommand(float(heightSize.get()),float(breadthSize.get()),float(angleSize.get()), float(widthSize.get()), penColor.get())
            cmd.draw(theTurtle)
            self.graphicsCommands.append(cmd)
        ...
        # This creates the button widget in the sideBar. The fill=tkinter.BOTH causes the button
        # to expand to fill the entire width of the sideBar.
        rectangleButton = tkinter.Button(sideBar, text = "Draw Rectangle", command=rectangleHandler)
        rectangleButton.pack(fill=tkinter.BOTH)             
        ...
        angleLabel = tkinter.Label(sideBar,text="angle")
        pointLabel.pack()
        angleSize = tkinter.StringVar()
        angleEntry = tkinter.Entry(sideBar,textvariable=angleSize)
        angleEntry.pack()
        angleSize.set(str(90))
